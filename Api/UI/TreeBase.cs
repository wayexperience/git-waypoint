...
namespace Unity.VersionControl.Git
{
    ...
    public abstract class TreeBase<TNode, TData> where TNode : class, ITreeNode where TData : struct, ITreeData
    {
        ...
        protected TNode AddNode(string path, string label, int level, bool isFolder, bool isActive, bool isHidden, bool isCollapsed, bool isSelected, bool isChecked, TData? treeData)
        {
            ...
            // Load() restores a node's checked state visually (via CheckState above) but does not go
            // through the toggle path, so the checked-files tracking would otherwise stay empty after a
            // rebuild - making GetCheckedFiles() return nothing even though the boxes look ticked. Keep
            // the tracking in sync here so selections (and the commit they drive) survive a refresh.
            if (isChecked && !isFolder)
            {
                AddCheckedNode(node);
            }
