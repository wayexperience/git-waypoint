...
namespace Unity.VersionControl.Git
{
	...
	public class ZipHelper : IZipHelper
	{
            ...
            CancellationToken token = default)
		{
			...
			// Prefer the OS `tar` for tarballs: the vendored SharpZipLib mishandles modern dugite-native
			// archives (many symlinks) and silently extracts nothing, leaving git/git-lfs missing. bsdtar
			// (macOS / Windows 10+) and GNU tar (Linux) extract reliably and keep symlinks + exec bits.
			if (archive.EndsWith(".tar.gz") || archive.EndsWith(".tgz") || archive.EndsWith(".tar"))
			{
				if (TryExtractWithSystemTar(archive, destDir, onStart, onProgress))
					return true;
				LogHelper.GetLogger<ZipHelper>().Warning("System tar failed/unavailable; falling back to SharpZipLib for {0}", archive);
			}
		...
		// Extract a tarball with the operating system's `tar`. Returns false (so callers can fall back)
		// if tar isn't found or exits non-zero. tar ships with macOS, Linux and Windows 10 1803+.
		private static bool TryExtractWithSystemTar(string archive, SPath outFolder,
			Action<string, long> onStart, Func<long, long, string, bool> onProgress)
		{
			try
			{
				onStart?.Invoke(archive, 0);
				var gzip = archive.EndsWith(".tar.gz") || archive.EndsWith(".tgz");
				// GUI-launched editors have a minimal PATH; use the absolute tar on Unix, and rely on
				// System32 being on PATH for Windows' bundled tar.exe.
				var tarExe = "tar";
				if (System.IO.File.Exists("/usr/bin/tar")) tarExe = "/usr/bin/tar";
				else if (System.IO.File.Exists("/bin/tar")) tarExe = "/bin/tar";
				var psi = new System.Diagnostics.ProcessStartInfo
				{
					FileName = tarExe,
					Arguments = (gzip ? "-xzf " : "-xf ") + "\"" + archive + "\" -C \"" + outFolder + "\"",
				...
				using (var p = System.Diagnostics.Process.Start(psi))
				{
					...
					if (p.ExitCode != 0)
					{
						LogHelper.GetLogger<ZipHelper>().Trace("tar exited {0}: {1}", p.ExitCode, err);
						return false;
					}
				}
				onProgress?.Invoke(1, 1, archive);
				return true;
			}
			catch (Exception ex)
			{
				LogHelper.GetLogger<ZipHelper>().Trace(ex, "system tar not available");
				return false;
			}
		}
