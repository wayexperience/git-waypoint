...
namespace Unity.VersionControl.Git
{
    ...
    public interface IRepositoryManager : IDisposable
    {
        ...
        ITask Pull(string remote, string branch, bool rebase);
    ...
    public class RepositoryManager : IRepositoryManager
    {
        ...
        {
            return Pull(remote, branch, false);
        }

        public ITask Pull(string remote, string branch, bool rebase)
        {
            var task = GitClient.Pull(remote, branch, rebase);
            return HookupHandlers(task, true);
        }
        ...
        public ITask UnlockFile(SPath file, string id, bool force)
        {
            var task = GitClient.Unlock(file, id, force)
                .Then(() => DataNeedsRefreshing?.Invoke(CacheType.GitLocks));
            return HookupHandlers(task, false);
        }
        ...
        public ITask DiscardChanges(GitStatusEntry[] gitStatusEntries)
        {
            ...
            // Classify entries up front (pure), then chain the work *sequentially* so that external
            // continuations (.ThenInUI/.FinallyInUI) fire after everything, and the status cache is
            // invalidated when done. The previous version appended the discard task dynamically from
            // inside the action body and never refreshed, so callers' continuations never ran and the
            // Changes view stayed stale until a manual refresh.
            ...
            var deleteTask = new ActionTask<GitStatusEntry[]>(taskManager, entries =>
                {
            ...
            {
                task = deleteTask.Then(GitClient.Discard(itemsToRevert))
