...
namespace Unity.VersionControl.Git.UI
{
    class Styles
    {
        ...
        // Owner-based lock shields, same scheme everywhere (project overlay, Changes and Locks panels):
        // green = locked by me, yellow = locked by me and modified, red+person = locked by someone else.
        public static Texture2D GetLockBadge(GitFileStatus status, bool lockedByMe)
        {
            if (!lockedByMe)
                return Utility.GetIcon("lock-other");

            return status == GitFileStatus.Modified
                ? Utility.GetIcon("lock-mine-modified")
                : Utility.GetIcon("lock-mine");
        }

        // Single source of truth for a file's badge so the project overlay and the Changes panel agree:
        // a merge conflict wins over a lock, a lock wins over a plain working-tree change.
        public static Texture2D GetStatusBadge(GitFileStatus status, bool isLocked, bool lockedByMe)
        {
            return GetStatusBadge(status, isLocked, lockedByMe, false);
        }

        // Priority for the single per-file badge: a merge conflict wins over "outdated" (a newer version
        // waiting on the server), which wins over a lock, which wins over a plain working-tree change.
        public static Texture2D GetStatusBadge(GitFileStatus status, bool isLocked, bool lockedByMe, bool isOutdated)
        {
            if (status == GitFileStatus.Unmerged)
                return Utility.GetIcon("conflicted");
            if (isOutdated)
                return Utility.GetIcon("outdated");
            return isLocked ? GetLockBadge(status, lockedByMe) : GetFileStatusIcon(status, false);
        }
