...
namespace Unity.VersionControl.Git.UI
{
    ...
    class Window : BaseWindow
    {
        ...
        private const string Title = "Git for Unity (legacy)";
        ...
        public override void DoProgressGUI()
        {
            ...
            {
                // While an automatic background refresh runs, still draw the (empty) bar area so the strip
                // stays visually consistent, but suppress the fill and message so the poll doesn't flash.
                bool quiet = BackgroundGitRefresh.IsQuiet;
                float value = quiet ? 0f : repositoryProgressValue;
                string message = quiet ? string.Empty : repositoryProgressMessage;

                var style = Styles.ProgressAreaBackStyle;
                style.Draw(rect1, false, false, false, false);
                Rect rect2 = new Rect(rect1.x, rect1.y, position.width * value, rect1.height);
                style = GUI.skin.FindStyle("ProgressBarBar");
                style.Draw(rect2, false, false, false, false);
                style = GUI.skin.FindStyle("ProgressBarText");
                style.Draw(rect1, message, false, false, false, false);
            }
        ...
        private void DoActionbarGUI()
        {
            GUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                if (hasRemote)
                {
                    ...
                    {
                        // Fetch button
                        if (GUILayout.Button(fetchButtonContent, Styles.ToolbarButtonStyle))
                            Fetch();

                        // Pull button - popup lets you choose merge vs rebase
                        ...
                        if (GUILayout.Button(pullButtonText, Styles.ToolbarButtonStyle))
                        {
                    ...
                    EditorGUI.BeginDisabledGroup(currentRemoteName == null || isTrackingRemoteBranch && statusAhead == 0);
                    {
                        ...
                        if (GUILayout.Button(pushButtonText, Styles.ToolbarButtonStyle))
                            Push();
                    }
                    EditorGUI.EndDisabledGroup();
                }
        ...
        // Live status to the right of the toolbar buttons: shows the background fetch and how many commits
        // are waiting to be pulled, so the toolbar reflects remote state at a glance.
        private void DoToolbarStatusGUI()
        {
            string status;
            bool highlight = false;
            if (!string.IsNullOrEmpty(repositoryProgressMessage))
            {
                // An operation is in flight (committing, pushing, unlocking, reading locks…): show it.
                status = repositoryProgressMessage;
            }
            else if (GitAutoFetch.IsFetching)
            {
                status = "Fetching…";
            }
            else if (hasRemote && isTrackingRemoteBranch && statusBehind > 0)
            {
                status = statusBehind == 1 ? "↓ 1 commit to pull" : String.Format("↓ {0} commits to pull", statusBehind);
                highlight = true;
            }
            else if (hasRemote)
            {
            ...
            var prev = GUI.color;
            if (highlight)
                GUI.color = new Color(1f, 0.78f, 0.31f);
            GUILayout.Label(status, ToolbarStatusStyle, GUILayout.ExpandHeight(true));
            GUI.color = prev;
        }

        private static GUIStyle toolbarStatusStyle;
        // Vertically centred label so the toolbar status lines up with the buttons next to it.
        private static GUIStyle ToolbarStatusStyle =>
            toolbarStatusStyle ?? (toolbarStatusStyle = new GUIStyle(EditorStyles.miniLabel) { alignment = TextAnchor.MiddleLeft });
        ...
        private void Pull(bool rebase)
        {
            // A merge pull needs a clean tree; a rebase pull uses --autostash, so it can run with
            // uncommitted changes (they're stashed and re-applied around the rebase).
            if (!rebase && hasItemsToCommit)
            {
                ShowNotification(new GUIContent("Commit your changes before pulling"));
            }
            else
            {
                ...
                Repository
                    ...
                    .FinallyInUI((success, e) =>
                    {
                        if (success)
                        {
                            ...
                            ShowNotification(new GUIContent(String.Format(Localization.PullSuccessDescription, currentRemoteName)));

                            AssetDatabase.Refresh();
                            LfsLocksModificationProcessor.RefreshOutdated(); // pulled: clear outdated badges
                        }
                        else
                        {
                            SetProgressMessage(Localization.MessagePullFailed, 100);
                            ShowNotification(new GUIContent("Pull failed - see Console"));
                            Debug.LogWarning("Git pull failed: " + e.Message);
                        }
                    })
                    .Start();
            }
            ...
            Repository
                ...
                .FinallyInUI((success, e) =>
                {
                    if (success)
                    {
                        ...
                        // Pushing is the "submit" step: drop my locks on files that are now in sync.
                        LfsLocksModificationProcessor.ReleaseLocksAfterPush();

                        ShowNotification(new GUIContent(String.Format(Localization.PushSuccessDescription, currentRemoteName)));
                    }
                    else
                    {
                        SetProgressMessage(Localization.MessagePushFailed, 100);
                        ShowNotification(new GUIContent("Push failed - see Console"));
                        Debug.LogWarning("Git push failed: " + e.Message);
                    }
                })
                .Start();
        }
            ...
            Repository
                ...
                .FinallyInUI((success, e) =>
                {
                    if (success)
                    {
                        ...
                        LfsLocksModificationProcessor.RefreshOutdated(); // recompute outdated files after a manual fetch
                    }
                    else
                    {
                        SetProgressMessage(Localization.MessageFetchFailed, 100);
                        ShowNotification(new GUIContent("Fetch failed - see Console"));
                        Debug.LogWarning("Git fetch failed: " + e.Message);
                    }
                })
                .Start();
        }
        ...
        private void SetProgressMessage(string message, long value, float timeout)
        {
            ...
            // This is a user-initiated operation: make sure a background "quiet" window doesn't hide it.
            BackgroundGitRefresh.Clear();
            if (repositoryProgress == null)
            ...
            // Reflect the message right away; the deferred copy in OnDataUpdate can lag a quick operation.
            repositoryProgressMessage = message;
            repositoryProgressValue = repositoryProgress.Percentage;
