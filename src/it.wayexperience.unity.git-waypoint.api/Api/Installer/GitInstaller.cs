...
namespace Unity.VersionControl.Git
{
    ...
    public class GitInstaller : TaskBase<GitInstaller.GitInstallationState>
    {
        ...
        private GitInstallationState FindGit(GitInstallationState state)
        {
            if (!state.GitIsValid)
            {
                // Prefer a Homebrew git over Apple's /usr/bin/git stub: the stub ships no git-lfs, which
                // auto-locking depends on. `which` would otherwise return the stub first.
                var gitPath = FindMacBinary("git");
                if (!gitPath.IsInitialized)
                    gitPath = new FindExecTask(TaskManager,
        ...
        // On macOS, probe the standard Homebrew locations directly so a full-featured Homebrew
        // git/git-lfs wins over Apple's bundled stub, independent of PATH ordering. Empty elsewhere.
        private SPath FindMacBinary(string name)
        {
            if (!platform.Environment.IsMac) return default(SPath);
            foreach (var dir in new[] { "/opt/homebrew/bin", "/usr/local/bin" })
            {
                var p = dir.ToSPath().Combine(name);
                if (p.FileExists()) return p;
            }
            return default(SPath);
        }
        ...
        private GitInstallationState FindGitLfs(GitInstallationState state)
        {
            if (!state.GitLfsIsValid)
            {
                // Same priority as git: pick up a Homebrew git-lfs before falling back to PATH lookup.
                var gitLfsPath = FindMacBinary("git-lfs");
                if (!gitLfsPath.IsInitialized)
                    gitLfsPath = new FindExecTask(TaskManager,
                                  platform.ProcessManager.DefaultProcessEnvironment, platform.Environment,
        ...
        private GitInstallationState ExtractGit(GitInstallationState state)
        {
            ...
            if (state.GitZipExists && !state.GitIsValid)
            {
                ...
                if (unzipTask.Successful)
                {
                    ...
                    // Validate the freshly extracted binaries so GitVersion/GitLfsVersion get populated
                    // (the UI shows them) and we confirm the bundle actually runs, instead of trusting blindly.
        ...
        public class GitInstallDetails
        {
            ...
            // Upstream desktop/dugite-native ships current Git + git-lfs with native macOS/Windows
            // arm64 builds; the old github-for-unity fork is frozen at Git 2.21 (2019, no Apple Silicon).
            public const string ManifestFeed = "https://api.github.com/repos/desktop/dugite-native/releases/latest";
