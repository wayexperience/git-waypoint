...
namespace Unity.VersionControl.Git
{
    ...
    public interface IGitClient
    {
        ...
        /// <summary>
        /// Executes `git pull` (optionally with --rebase) to pull changes.
        /// </summary>
        /// <param name="remote">The remote to pull from</param>
        /// <param name="branch">The branch to pull</param>
        /// <param name="rebase">Rebase local commits on top of the upstream instead of merging</param>
        /// <returns>String output of git command</returns>
        ITask<string> Pull(string remote, string branch, bool rebase);
        ...
        /// <summary>
        /// Executes `git lfs unlock --id` to unlock a file by its lock id (avoids a server lookup).
        /// </summary>
        /// <param name="file">The file to unlock (used only as a fallback when id is empty)</param>
        /// <param name="id">The lock id; when empty, falls back to unlocking by path</param>
        /// <param name="force">If force should be used</param>
        /// <returns>String output of git command</returns>
        ITask<string> Unlock(SPath file, string id, bool force);
    ...
    public class GitClient : IGitClient
    {
        ...
        ///<inheritdoc/>
        public ITask<string> Pull(string remote, string branch, bool rebase)
        {
            return new GitPullTask(platform, remote, branch, rebase, Token)
                .Configure(platform.ProcessManager);
        }
        ...
        ///<inheritdoc/>
        public ITask<string> Unlock(SPath file, string id, bool force)
        {
            if (string.IsNullOrEmpty(id))
                return Unlock(file, force);
            return new GitUnlockTask(platform, id, force, Token)
                .Configure(platform.ProcessManager);
        }
