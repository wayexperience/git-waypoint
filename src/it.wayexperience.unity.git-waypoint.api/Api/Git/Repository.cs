...
namespace Unity.VersionControl.Git
{
    ...
    public class Repository : IEquatable<Repository>, IRepository
    {
        ...
        public ITask Pull(bool rebase) => repositoryManager.Pull(CurrentRemote.Value.Name, CurrentBranch?.Name, rebase);
    ...
    public class User : IUser
    {
        ...
        {
            // Write, then read the config BACK so Name/Email reflect what git actually stored, not the
            // values we hoped to write. An optimistic update hid failed writes (banner cleared while
            // `git config --global` stayed empty).
            gitClient.SetConfigNameAndEmail(name, email)
                     .Then(gitClient.GetConfigUserAndEmail())
                     .Catch(ex => { Logger.Error(ex, "Failed to set git user.name/user.email (git config --global)"); return true; })
                     .ThenInUI(value => {
                         Name = value != null ? value.Name : string.Empty;
                         Email = value != null ? value.Email : string.Empty;
                         Logger.Warning("Git identity after save: requested name='{0}' email='{1}'; git reports name='{2}' email='{3}'", name, email, Name, Email);
                     })
                     .Start();
        }
