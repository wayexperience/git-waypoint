...
namespace Unity.VersionControl.Git
{
    ...
    public class RepositoryWatcher : IRepositoryWatcher
    {
        ...
        public void Start()
        {
            ...
            {
                // Native file-system watcher not available on this platform (e.g. no arm64 build on
                // Apple Silicon). Degrade gracefully instead of failing "setting up Git": skip the
                // .git watcher. Unity's AssetPostprocessor + manual refresh keep status current.
                Logger.Warning("NativeInterface is null; running without the native repository watcher");
                return;
            }
            ...
            if (paths.IsWorktree)
            {
                ...
                {
                    Logger.Warning("Worktree NativeInterface is null; skipping worktree watcher");
                    return;
                }
        ...
        public int CheckAndProcessEvents()
        {
            if (nativeInterface == null)
            {
                return 0; // native watcher unavailable; nothing to process
            }
