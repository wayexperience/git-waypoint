...
The same applies to this project, which is split into two packages - `it.wayexperience.unity.git-waypoint.api` - the Git client library; and `it.wayexperience.unity.git-waypoint.ui` - a Unity Git UI based on the Git for Unity UI, which examplifies how to use the Git client library.
