...
if [[ x"$OS" == x"Windows" ]]; then
  ...
  pushd "$srcdir/it.wayexperience.unity.git-waypoint" >/dev/null 2>&1
  tgz="$(npm pack -q)"
  mv -f $tgz $targetdir/$tgz  
  pushd "$srcdir/it.wayexperience.unity.git-waypoint.tests" >/dev/null 2>&1
  tgz="$(npm pack -q)"
  mv -f $tgz $targetdir/$tgz  
...
EOL
  ...
  for j in `ls -d *`; do
    ...
    if [[ x"$j" == x"it.wayexperience.unity.git-waypoint" ]]; then continue; fi
    if [[ x"$j" == x"it.wayexperience.unity.git-waypoint.tests" ]]; then continue; fi
