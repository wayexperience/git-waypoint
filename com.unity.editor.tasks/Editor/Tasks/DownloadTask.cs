	...
	public class Downloader : TaskQueue<string, DownloadData>
	{
			...
			Func<long, long, bool> onProgress = null)
		{
			...
			using (var webResponse = (HttpWebResponse)webRequest.GetResponseWithoutException())
			{
				...
				{
					// Read to EOF instead of capping at Content-Length. With AutomaticDecompression on,
					// gzip responses (e.g. GitHub's releases JSON) report the COMPRESSED length, which
					// would truncate the decompressed stream mid-document. The HTTP body ends at EOF, so
					// this is correct for both compressed JSON and plain binary (tarball) downloads.
					var buffer = new byte[8192];
					var total = responseLength > 0 ? bytes + responseLength : 0;
					int read;
					while ((read = responseStream.Read(buffer, 0, buffer.Length)) > 0)
					{
						destinationStream.Write(buffer, 0, read);
						bytes += read;
						if (!(onProgress?.Invoke(bytes, total > 0 ? total : bytes) ?? true))
							return false;
					}
					return true;
				}
