...
try {
	Push-Location (Join-Path $srcDir "it.wayexperience.unity.git-waypoint")
...
try {
	Push-Location (Join-Path $srcDir "it.wayexperience.unity.git-waypoint.tests")
	$package = Invoke-Command -Fatal { & npm pack -q }
	$package = "$package".Trim()
	$tgt = Join-Path $targetDir $package
	Move-Item $package $tgt -Force
...
Get-ChildItem -Directory $srcDir | % {
    ...
    if ("$($_.Name)" -ne "it.wayexperience.unity.git-waypoint" -and "$($_.Name)" -ne "it.wayexperience.unity.git-waypoint.tests" -and (Test-Path "$srcDir\$($_)\package.json")) {
        try {
