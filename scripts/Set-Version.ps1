...
if ($NewVersion -ne '' -and !($BumpMajor -or $BumpMinor -or $BumpPatch -or $BumpBuild)) {
	$versionFile = "$rootDirectory\src\it.wayexperience.unity.git-waypoint.ui\version.json"
	Set-Version $versionFile $NewVersion

	$versionFile = "$rootDirectory\src\it.wayexperience.unity.git-waypoint.api\version.json"
	Set-Version $versionFile $NewVersion
} else {
	$versionFile = "$rootDirectory\src\it.wayexperience.unity.git-waypoint.ui\version.json"
